...
const getTransactions=async({page,from,to})=>{
  try {
    let route=`api/report/system-transactions?limit=10&page=${page+1}&sort=-date`;
    if(from&&to){
...
const TransactionsReport = () => {
  ...
  const {data,isLoading,error}=useQuery({
    queryKey:[`system-transactions-report-${from+to}-${[page]}`],
    queryFn:async()=>getTransactions({page,from,to})
  })
